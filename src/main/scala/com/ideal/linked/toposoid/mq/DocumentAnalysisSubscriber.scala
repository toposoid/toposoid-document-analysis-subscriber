...
object DocumentAnalysisSubscriber extends App {
  ...
  private val settings = SqsSourceSettings().withCloseOnEmptyReceive(false)
  ...
  val messages = SqsSource(queueUrl, settings)
    ...
    //.runWith(Sink.foreach { res: SqsAckResult =>
      ...
      def convertKnowledge(knowledge: Knowledge): Knowledge = {
        val knowledgeForImages: List[KnowledgeForImage] = knowledge.knowledgeForImages.map(y => {
          Option(y.id) match {
            case Some(x) => {
              if(x.strip().equals("")) KnowledgeForImage(java.util.UUID.randomUUID().toString, y.imageReference)
              else y
            }
            case None => {
        ...
        val knowledgeForTables: List[KnowledgeForTable] = knowledge.knowledgeForTables.map(y => {
          Option(y.id) match {
            case Some(x) => {
              if (x.strip().equals("")) KnowledgeForTable(java.util.UUID.randomUUID().toString, y.tableReference)
              else y
            }
            case None => {
      ...
      def makeKnowledgeSentenceSet(proposition:List[Knowledge]): KnowledgeSentenceSet = {
      ...
      def addNonSentenceSectionResult(documentRegistration: DocumentRegistration, propositions:List[List[Knowledge]]) = Try {
    ...
    //==================================================================================================================================
      ...
      def analyzePdfDocument(document :Document, transversalState:TransversalState):Propositions = {
        try {
      ...
      def getSurfaceIndex(label:String, predicateArgumentStructures:List[PredicateArgumentStructure], transliterator:Transliterator):(Int, String) = {
      ...
      def linkContentLabelIndex(analyzedPropositionPair: AnalyzedPropositionPair): AnalyzedPropositionPair = {
